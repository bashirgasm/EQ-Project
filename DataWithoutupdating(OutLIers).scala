...
// Define and Calculation Of Outliers  
...
val 
analysis1 =finaldata.groupBy("POIID").agg(avg("min(distance)") as "Avg(Without Outliers)", stddev("min(distance)") as "STD(Without Outliers)")
