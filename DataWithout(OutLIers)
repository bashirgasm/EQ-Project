...
val data2 =distdata.groupBy("_ID").min("distance") // Display Minimum Distance By ID
val data3 =distdata.groupBy("POIID").min("distance") // Display Minimum Distance By POIID
// Display Data after Joining (The target Data) and Filterring Outliers Limits
val finaldata = distdata.join(data2,distdata("_ID") === data2("_ID") && distdata("distance")===data2("min(distance)")).drop(data2("_ID"))
.select("_ID","POIID","min(distance)").where(col("min(distance)") > -392.63 && col("min(distance)") < 1037.78)   
// Display Average and standard deviations data without outliers
val analysis1 =finaldata.groupBy("POIID").agg(avg("min(distance)") as "Avg(Without Outliers)", stddev("min(distance)") as "STD(Without Outliers)")
...
// Define and Calculation Of Outliers Boundroes and pass the values later to The Target Data
val analysis2 = finaldata.filter(s"min(distance)  > lowerRange  && min(distance) < upperRange")
//  Radious and Density with outliers <Going On > 
val radiousdata = finaldata.groupBy("POIID").agg(max("min(distance)") as "Radious with outliers", count("min(distance)") as "Count With outliers")
val radiousdensity = radiousdata.select(expr("radiousdata(Count With outliers)/radiousdata(Radious with outliers)**2*np.pi)")).show
